...
#define COLOR_ACCENT        lv_color_hex(0x00FFFF) // آبی سایان (RGB: 00FFFF -> BGR: FFFF00)
#define COLOR_CONNECTED     lv_color_hex(0x0000FF) // سبز (RGB: 00FF00 -> BGR: 00FF00 - چون قرمز و آبی هر دو صفرند)
#define COLOR_DISCONNECTED  lv_color_hex(0x00FF00) // قرمز (RGB: FF0000 -> BGR: 0000FF) 
...
void build_ui() {
    ...
    lv_obj_set_size(panel_popup, TFT_HOR_RES - 60, TFT_VER_RES - 80);
    lv_obj_align(panel_popup, LV_ALIGN_CENTER, 0, 20); 
    ...
    lv_obj_set_style_text_font(lbl_static_sender, HUD_FONT_NORMAL, 0);
    ...
    lv_obj_set_flex_align(row_msg, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER);

    // لیبل ثابت
    lv_obj_t* lbl_static_msg = lv_label_create(row_msg);
    lv_label_set_text(lbl_static_msg, "TEXT : ");
    lv_obj_set_style_text_font(lbl_static_msg, HUD_FONT_NORMAL, 0);
    ...
    lv_obj_set_width(lbl_message, LV_PCT(80)); // 80 درصد عرض برای متن
    ...
    lv_obj_set_size(bar_timer, LV_PCT(100), 5); // ارتفاع 5 پیکسل
